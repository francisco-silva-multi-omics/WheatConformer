...
PHENO_OUT = cwd / "phenotypes"
MAS_OUT = cwd / "genotype_panels" / "mas"
ANNOT_OUT = cwd / "functional_annotation"
...
def build_gid_lookup() -> pd.DataFrame:
    manifest = pd.read_csv(
        cwd / "metadata_outputs" / "all_trials_genotype_manifest_resolved.tsv",
        sep="\t",
        dtype=str,
        low_memory=False,
    )
...
def build_modeling_ready_phenotypes() -> None:
    print("Construyendo tabla de fenotipos lista para modelado desde GrnYld + MeanVal")
    out_path = PHENO_OUT / "modeling_ready_phenotypes.tsv"
    if out_path.exists():
        out_path.unlink()
...
def build_collapsed_modeling_phenotypes() -> None:
    print("Colapsando registros de fenotipos duplicados a medias de entrada de modelado")
...
def build_mas_favorable_dosage() -> None:
    print("Recodificando marcadores MAS a dosis de alelo favorable")
...
def build_annotation_and_graph_gap_reports() -> None:
    print("Construyendo anotación funcional e informes de evidencia/integración de grafo")
