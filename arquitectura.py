...
HMP = cwd / "Genotypic_data_from_CIMMYT_bread_wheat_breeding_lines" / "F_MAF0.01_Miss50_Het10-Merged.all.discover.lines.and.selection.candidates.vcf.imputed.CIMMYT.2022.hmp.txt"
DARTAG_DIR = cwd / "Genotypic_data_(DArTAG_panel_2)_for_the_IBWSN_and_SAWSN"
MAS_FILES = [
    cwd / "57IBWSN,_42SAWSN,_and_35HRWSN_-_Gene-based_marker_data_for_marker-assisted_selection" / "57IBWSN,_42SAWSN,_35HRWSN_results.xlsx",
    cwd / "58IBWSN_and_43SAWSN_-_Gene-based_marker_data_for_marker-assisted_selection" / "58IBWSN-43SAWSN_results.xlsx",
]
HAPLOBLOCKS = cwd / "Haplotype-based_genome-wide_association_study" / "Haplotype_blocks_EYT2011-12_to_EYT2017-18.csv"
DARTSEQ_LANDRACE_DIR = cwd / "DArTseq-derived_SNPs_for_wheat_Mexican_landrace_accessions"


def rel(path: Path) -> str:
    return str(path.resolve()).replace(str(cwd.resolve()) + os.sep, "")


def ensure_dirs() -> None:
    for path in [
        cwd / "genotype_panels" / "hmp",
        cwd / "genotype_panels" / "mas",
        cwd / "genotype_panels" / "dartag",
        cwd / "genotype_panels" / "dartseq_landrace",
        cwd / "genotype_panels" / "haploblocks",
        cwd / "functional_annotation",
        cwd / "phenotypes",
        cwd / "environment",
    ]:
...
def build_hmp_outputs() -> pd.DataFrame:
    print("Construyendo metadatos de marcadores HMP, matriz de muestras coincidentes y K_HMP.npy")
    hmp_out = cwd / "genotype_panels" / "hmp"
    usable = pd.read_csv(cwd / "metadata_outputs" / "usable_trial_to_canonical_hmp_matches.tsv", sep="\t", dtype=str)
...
def rebuild_hmp_kernel_from_parquet() -> None:
    ...
    print("Diagonal promedio:", np.mean(np.diag(K)))


def build_hmp_qcfiltered_outputs() -> None:
    out = cwd / "genotype_panels" / "hmp"
    ...
    print("Muestras originales:", M.shape[0])
    print("Marcadores originales:", M.shape[1])
    print("Muestras mantenidas:", int(keep_samples.sum()))
    print("Marcadores mantenidos:", int(keep_markers.sum()))
    print("Marcadores de bajo MAF eliminados:", int((maf < maf_min).sum()))
    print("Marcadores de alta heterocigosis eliminados:", int((marker_het > marker_het_max).sum()))
    print("Muestras de alta heterocigosis eliminadas:", int((~keep_samples).sum()))
    print("Matriz filtrada:", X_filt.shape)
    print("Forma de K:", K.shape)
    print("Diagonal promedio de K:", float(np.mean(np.diag(K))))


def build_dartag_outputs() -> pd.DataFrame:
    print("Construyendo manifiestos de DArTAG y parquet codificado")
    out_dir = cwd / "genotype_panels" / "dartag"
    manifests = []
...
def build_dartseq_landrace_outputs() -> pd.DataFrame:
    print("Construyendo manifiesto de panel DArTseq de landrace mexicano, metadatos de marcadores y parquet marcador-por-muestra")
    out_dir = cwd / "genotype_panels" / "dartseq_landrace"
...
def build_mas_outputs() -> pd.DataFrame:
    print("Construyendo panel de muestras MAS y llamadas de marcadores")
    out_dir = cwd / "genotype_panels" / "mas"
    samples, metas, calls = [], [], []
...
def build_haploblocks() -> pd.DataFrame:
    print("Construyendo TSV de bloques de haplotipos")
    out_path = cwd / "genotype_panels" / "haploblocks" / "eyt2011_2018_haploblocks.tsv"
...
def build_functional_annotations(
    ...
    print("Construyendo tablas de anotación funcional")
    out_dir = cwd / "functional_annotation"
    ...
    hmp_meta = pd.read_csv(cwd / "genotype_panels" / "hmp" / "hmp_marker_metadata.tsv", sep="\t", dtype=str)
...
def build_phenotypes_and_environment() -> None:
    print("Construyendo TSVs combinados de fenotipo y ambiente")
    collect_trial_tables("MeanVal", cwd / "phenotypes" / "all_meanval.tsv")
    collect_trial_tables("GrnYld", cwd / "phenotypes" / "all_grnyld.tsv")
    collect_trial_tables("RawData", cwd / "phenotypes" / "all_rawdata.tsv")
    env = collect_trial_tables("EnvData", cwd / "environment" / "envdata.tsv")
    collect_trial_tables("loc", cwd / "environment" / "locdata.tsv")
    if env.empty or "Value" not in env.columns:
        write_tsv(pd.DataFrame(), cwd / "environment" / "env_kernel.tsv")
        return
    ...
    write_tsv(kernel.reset_index(drop=True), cwd / "environment" / "env_kernel.tsv")
