...
## 13. Kernel diagnostic figures

The reproducible Phase 13 generator is `generate_kernel_diagnostic_figures.py`. Its local output index is `kernel_diagnostics/FIGURE_INDEX.md`; generated PNGs are under `figures/kernel_diagnostics/`, with numerical summaries and nearest-neighbor tables under `kernel_diagnostics/`.

For every discovered square kernel, the suite records the full dimension and deterministic sample size, eigenvalue spectrum, diagonal and off-diagonal distributions, explicit-order and randomly permuted heatmaps, clustered heatmap, effective rank, condition number, nearest neighbors, and nearest/duplicate sampled pairs. Cross-kernel comparisons require shared explicit IDs; comparisons with fewer than 10 shared IDs are retained as insufficient evidence in the TSV and masked in the summary heatmap. Environment kernels also receive clustered annotation figures for country, cycle, temperature, precipitation, VPD, heat, and drought descriptors, plus sidecar tables with exact trial and location labels.

Local coverage currently includes HMP and GBS genomic kernels, recovered IWYP linear/RBF kernels, generic environmental components and combined `K_E`, and explicit smoke `K_G`, `K_E`, and Hadamard `K_GxE`. The reviewed conflict-free production `K_A`, server trait-specific environment kernels, and full production `K_GxE` factors/ledger are not present locally, so those figures must be generated by running the same command on the server. The generated evidence under `audit/` is ignored by Git; the tracked generator and tests are the reproducible source.

## 13.1 Synthetic-test results

Run `.audit-venv/Scripts/python -m pytest tests/test_forensic_kernel_math.py tests/test_kernel_diagnostic_figures.py -q`. Tests cover analytical VanRaden, additive pedigree, environment standardization/nonfinite behavior, GxE Hadamard indexing, join cardinality, split leakage semantics, sampled diagnostics, and environment annotation.
...
.\.audit-venv\Scripts\python.exe -m audit.generate_kernel_diagnostic_figures --root . --out-dir audit/kernel_diagnostics --figures-dir audit/figures/kernel_diagnostics --max-sample 512 --heatmap-size 128 --seed 20260715
...
```bash
python audit/run_forensic_audit.py --root . --trial-root TRIALS_AND_NURSERIES --genotypic-root GENOTYPIC_DATA --out-dir audit --skip-source-inventory
python audit/validate_server_artifacts.py --root . --out-dir audit/server_artifacts
python -m audit.generate_kernel_diagnostic_figures --root . --out-dir audit/kernel_diagnostics --figures-dir audit/figures/kernel_diagnostics --max-sample 512 --heatmap-size 128 --seed 20260715
...
Kernel diagnostic failures: 0 for the locally discovered matrices under the numerical tolerance recorded per kernel. See `kernel_diagnostics/kernel_sampled_diagnostics_summary.tsv`, `kernel_diagnostics/cross_kernel_sampled_alignments.tsv`, and `kernel_diagnostics/FIGURE_INDEX.md`. This numerical status does not override the semantic K_A and generic K_E defects documented above.
