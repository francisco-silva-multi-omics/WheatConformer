...
def split_indices(n: int, seed: int, val_fraction: float, test_fraction: float):
...
def main() -> None:
    ...
    train_idx, val_idx, test_idx = split_indices(n, args.seed, args.val_fraction, args.test_fraction)
    ...
    split_df = pd.concat(
        [
            pd.DataFrame({"window_index": train_idx, "split": "train"}),
            pd.DataFrame({"window_index": val_idx, "split": "val"}),
            pd.DataFrame({"window_index": test_idx, "split": "test"}),
        ],
        ignore_index=True,
    )
