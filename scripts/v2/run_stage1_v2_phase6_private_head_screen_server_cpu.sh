...
if [[ ! -f "$CODE/server_training_pipeline/stage1_v2_phase6_private_head_screen_protocol_v1.json" ]]; then
  echo "Missing Stage-1 v2 private-head implementation under: $CODE" >&2
  exit 1
fi

STATE="$DATA/audit/v2/stage1_v2_phase6_private_head_screen_server_cpu_v1"
...
if [[ -s "$PID_FILE" ]]; then
  ...
  if [[ -n "$OLD_PID" ]] && kill -0 "$OLD_PID" 2>/dev/null; then
    echo "Stage-1 v2 private-head screen is already running: pid=$OLD_PID" >&2
    exit 1
  fi
fi
...
echo "FREEZE fixed-contract private-head phase_1 screen"
"$PYTHON" -m scripts.v2.freeze_stage1_v2_phase6_private_head_screen \
  --root "$DATA" \
  --code-root "$CODE"

STAMP="$(date -u +%Y%m%d_%H%M%S)"
LOG="$DATA/logs/stage1_v2_phase6_private_head_phase1_${STAMP}.nohup.log"
nohup setsid "$PYTHON" -m scripts.v2.run_stage1_v2_phase6_private_head_screen \
  --root "$DATA" \
  --code-root "$CODE" \
