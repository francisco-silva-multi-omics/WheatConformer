...
LOCK = Path(
    "audit/v2/stage1_v2_phase6_private_head_screen_v1/PRIVATE_HEAD_SCREEN_LOCK.json"
)
SOURCE_RUNS = Path(
...
def build_grid(root: Path, protocol: dict[str, Any]) -> pd.DataFrame:
    ...
    if len(states) != int(scope["state_count"]) or not states["state_id"].is_unique:
        raise ValueError(f"Expected five private-head states; observed={len(states)}")
    seeds = []
    for state_id in states["state_id"]:
        source = root / SOURCE_RUNS / state_id / SOURCE_REFERENCE / "run_metadata.json"
        if not source.is_file():
            raise FileNotFoundError(f"Private-head reference is missing: {source}")
        metadata = read_json(source)
        if metadata.get("status") != "PASS":
            raise ValueError(f"Private-head reference is not certified: {state_id}")
...
def run_complete(
    ...
    implementation_fields = {
        "private_head_protocol": "protocol_sha256",
        ...
        "source_trait_balance_decision": "trait_balance_decision_sha256",
    }
    if any(
        ...
        return False
    if int(metadata.get("decoder_variable_count", 0)) <= 0:
        return False
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Run the Stage-1 v2 private-head Phase-1 screen"
    )
    ...
    if lock.get("status") != "PASS_FROZEN_STAGE1_V2_PHASE6_PRIVATE_HEAD_SCREEN_V1":
        raise ValueError("Private-head screen is not frozen")
    for artifact, relative_path in {
        "private_head_protocol": PROTOCOL,
        "source_trait_balance_protocol": Path(
        ...
        if sha256_file(code_root / relative_path) != lock["artifacts"].get(artifact):
            raise ValueError(f"Private-head frozen implementation drift: {artifact}")
    if sha256_file(
        root
        / "model_kernels/stage1_v2_phase6_trait_balance_screen_v1/phase_1/"
        "TRAIT_BALANCE_PHASE1_DECISION.json"
    ) != lock["artifacts"]["source_trait_balance_decision"]:
        raise ValueError("Private-head parent decision drift")
    ...
    output = root / OUTPUT
    output.mkdir(parents=True, exist_ok=True)
    grid.to_csv(output / "private_head_state_grid.tsv", sep="\t", index=False)
    write_json(
        output / "private_head_status.json",
        {
            "status": "RUNNING",
    ...
    total = len(grid) * len(new_candidates)
    print(
        f"RUN Stage-1 v2 private-head phase_1; states={len(grid)} "
    ...
    with ThreadPoolExecutor(max_workers=args.workers) as executor:
        ...
        for future in as_completed(futures):
            ...
            complete += 1
            print(
                f"[{complete}/{total}] DONE {row['state_id']} {candidate}",
                flush=True,
            )
            gc.collect()
    ...
    candidate_guards = paired_guards.loc[
        ~paired_guards["candidate"].eq(REFERENCE)
    ]
    checks = {
        ...
        "decoder_policies_active": candidate_runs["decoder_variable_count"].gt(0).all()
        and candidate_runs["decoder_parameter_count"].gt(0).all(),
        "decoder_policies_distinct": candidate_runs["decoder_policy_sha256"].nunique()
        == 2,
        "authoritative_row_mass_unchanged": candidate_runs[
            "authoritative_row_mass_changed"
        ].eq(False).all(),
        "authoritative_row_mass_matched_between_candidates": candidate_runs.groupby(
            "state_id"
        )["authoritative_loss_weight_sha256"].nunique().eq(1).all(),
        "factor_backbone_unchanged": candidate_runs["factor_backbone_changed"].eq(
            False
        ).all(),
    ...
    artifacts = {
        "private_head_runs.tsv": runs,
        "private_head_trait_metrics.tsv": traits,
        "private_head_guard_metrics.tsv": guards,
        "private_head_paired_metrics.tsv": paired,
    ...
    status = (
        ...
        else "PASS_STAGE1_V2_PHASE6_PRIVATE_HEAD_PHASE1_COMPLETE_NO_ADVANCE"
    )
    if failed:
        status = "FAIL_STAGE1_V2_PHASE6_PRIVATE_HEAD_PHASE1_INTEGRITY"
    result = {
        "status": status,
        "protocol_version": protocol["protocol_version"],
    ...
    write_json(output / "PRIVATE_HEAD_PHASE1_DECISION.json", result)
    write_json(output / "private_head_status.json", result)
    print(json.dumps(result, indent=2, sort_keys=True), flush=True)
    if failed:
        raise SystemExit(f"Private-head phase_1 integrity failed: {failed}")
