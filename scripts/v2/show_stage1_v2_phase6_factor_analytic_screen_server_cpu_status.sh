...
RUNS="$DATA/trained_models/stage1_v2_phase6_factor_analytic_screen_v1_runs"
REPLAY_RUNS="$DATA/trained_models/stage1_v2_phase6_factor_analytic_screen_v1_same_seed_replay_runs"
STATUS="$DATA/model_kernels/stage1_v2_phase6_factor_analytic_screen_v1/phase_1/factor_analytic_status.json"
...
for path in pathlib.Path(sys.argv[1]).glob("*/*/run_metadata.json"):
    ...
    if (
        ...
        and value.get("factor_analytic_rank") in {2, 4}
        and value.get("factor_analytic_parameter_count", 0) > 0
