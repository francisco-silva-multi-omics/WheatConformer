...
SINGLE_STEP_DIR="${SINGLE_STEP_OUT_DIR:-model_kernels/single_step_H_v1}"
SCREEN_DIR="${SINGLE_STEP_SCREEN_DIR:-model_kernels/single_step_H_inner_screen_v1}"
MODELS_DIR="${SINGLE_STEP_MODELS_DIR:-trained_models/single_step_H_inner_screen_v1_runs}"
...
for required in "$MANIFEST" "$PLAN" "$SINGLE_STEP_DIR/single_step_screen_preparation.json"; do
  [[ -s "$required" ]] || { echo "Required single-step screen input is missing: $required" >&2; exit 2; }
done
...
echo "Single-step screen directory: $SCREEN_DIR"
