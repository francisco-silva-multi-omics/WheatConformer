...
This maps top-level folders and converts spaces to underscores. With `--mode symlink`, the repo points to the raw folders without duplicating data. With `--mode copy`, the folders are copied. The 80k diversity folder is renamed to `80k`.

Before preparing the workspace, validate that the main raw groups are present:
