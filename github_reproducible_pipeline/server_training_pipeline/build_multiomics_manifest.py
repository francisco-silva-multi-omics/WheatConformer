...
def main() -> None:
    ...
    pair_cols = ["sample_id", "assay", "mark", "condition", "tissue", "replicate"]
    manifest["pair_key"] = manifest[pair_cols].fillna("").astype(str).agg("|".join, axis=1)
    beds = manifest[manifest["file_type"].eq("peak_bed")].copy()
    exact_candidates = beds.groupby("pair_key")["path"].agg(list).to_dict()
    sample_candidates = beds.groupby("sample_id")["path"].agg(list).to_dict()

    paired = []
    statuses = []
    for row in manifest.itertuples(index=False):
        if row.file_type != "bigwig":
            paired.append("")
            statuses.append("not_bigwig")
            continue
        exact = exact_candidates.get(row.pair_key, [])
        if len(exact) == 1:
            paired.append(exact[0])
            statuses.append("exact_metadata_pair")
            continue
        sample = sample_candidates.get(row.sample_id, [])
        if len(sample) == 1:
            paired.append(sample[0])
            statuses.append("unique_sample_fallback")
        elif len(exact) > 1:
            paired.append("")
            statuses.append("ambiguous_exact_metadata_pair")
        elif len(sample) > 1:
            paired.append("")
            statuses.append("ambiguous_sample_pair")
        else:
            paired.append("")
            statuses.append("no_peak_pair")
    manifest["paired_peak_bed"] = paired
    manifest["peak_pair_status"] = statuses
