...
The `.bw` and `.bed` files are used for sequence-to-signal training after coordinate/signal QC. BigWig labels use `log1p` on nonnegative signal and per-track p95 scaling calculated from positive transformed bins, so sparse zero bins do not disable scaling. The FPKM/count `.txt` file is cataloged in the manifest as a gene-expression matrix, but it is not used by the window-level Enformer-like trainer yet. It is useful for a later gene-level expression head once gene coordinates are added.

Regulatory training defaults to chromosome-held-out train/validation/test splits. Do not replace this with random window splitting for final results because nearby or overlapping windows can leak sequence context across splits.
...
The TensorFlow multikernel runner automatically includes `K_A`, `K_AE`, `K_z`, and `K_zE` when their kernel and order files exist. The validation/ablation suite also accepts:
