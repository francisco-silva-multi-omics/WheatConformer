...
def aligned_optional_factors(kernel_path: Path, order_path: Path, compact_g_order_path: Path, rank: int) -> np.ndarray:
    ...
    candidates = ["sample_id", "panel_sample_id", "canonical_germplasm_key"]
    order_col = next((c for c in candidates if c in order.columns), None)
    compact_col = next((c for c in candidates if c in compact.columns), None)
    if order_col is None or compact_col is None:
        raise SystemExit(f"Could not identify ID columns for optional kernel {kernel_path}")
    mapper = {sample_id: i for i, sample_id in enumerate(order[order_col].astype(str).str.strip())}
    wanted = compact[compact_col].astype(str).str.strip()
    if not wanted.isin(mapper).all():
        raise SystemExit(f"Optional kernel {kernel_path} does not cover all compact genotypes")
    idx = wanted.map(mapper).to_numpy(dtype=np.int64)
    aligned = np.asarray(K[np.ix_(idx, idx)], dtype=np.float64)
    aligned = (aligned + aligned.T) / 2.0
    vals, vecs = np.linalg.eigh(aligned)
    order_idx = np.argsort(vals)[::-1]
    vals = vals[order_idx]
    vecs = vecs[:, order_idx]
    keep = vals > 1e-8
    vals = vals[keep][:rank]
...
def build_features(
    ...
    if "RBFE" in terms:
        ...
        Es = E[ei, : min(rank_ge_e, E.shape[1])]
        parts.append((Gs[:, :, None] * Es[:, None, :]).reshape(len(gi), -1))
    for name, factors in (("A", A), ("Z", Z)):
        if name in terms:
            if factors is None:
                raise ValueError(f"Ablation requests {name} but its kernel was not supplied")
            parts.append(factors[gi])
        interaction = name + "E"
        if interaction in terms:
            if factors is None:
                raise ValueError(f"Ablation requests {interaction} but its kernel was not supplied")
            Fs = factors[gi, : min(rank_ge_g, factors.shape[1])]
            Es = E[ei, : min(rank_ge_e, E.shape[1])]
            parts.append((Fs[:, :, None] * Es[:, None, :]).reshape(len(gi), -1))
...
def main() -> None:
    ...
    parser.add_argument("--rank-a", type=int, default=48)
    parser.add_argument("--rank-z", type=int, default=48)
    ...
    A = aligned_optional_factors(args.k_a, args.k_a_order, args.k_g_order, args.rank_a) if args.k_a and args.k_a_order else None
    Z = aligned_optional_factors(args.k_z, args.k_z_order, args.k_g_order, args.rank_z) if args.k_z and args.k_z_order else None
    if bool(args.k_a) != bool(args.k_a_order):
        raise SystemExit("Provide both --k-a and --k-a-order")
    if bool(args.k_z) != bool(args.k_z_order):
    ...
    if A is not None and args.ablation is None:
        ablations.extend(["A", "G+E+A", "G+E+A+GE+AE"])
    if Z is not None and args.ablation is None:
        ablations.extend(["Z", "G+E+Z", "G+E+Z+GE+ZE"])
    if A is not None and Z is not None and args.ablation is None:
        ablations.append("G+RBF+E+GE+RBFE+A+AE+Z+ZE")
    ...
    for repeat in range(args.repeats):
        for split_mode, group_col in split_cols.items():
            ...
            for ablation in ablations:
                X = build_features(ablation, G, E, G_RBF, A, Z, gi, ei, args.rank_ge_g, args.rank_ge_e)
