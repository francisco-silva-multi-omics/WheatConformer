...
def validate_kernel(
    ...
    required: bool,
    sample_n: int,
    expect_mean_diag_one: bool = False,
) -> dict[str, Any] | None:
    ...
    if len(diag) and np.all(np.isfinite(diag)):
        ...
        if expect_mean_diag_one:
            mean_diag = float(np.mean(diag))
            scaled = abs(mean_diag - 1.0) <= 0.10
            add_report(
                rows,
                f"{component}_kernel_mean_diag_one",
                "PASS" if scaled else ("FAIL" if required else "WARN"),
                kernel_path,
                f"sampled_mean_diagonal={mean_diag:.6g}; tolerance=0.10",
                required,
            )
    else:
    ...
    if len(sym_idx):
        ...
        eigenvalues = np.linalg.eigvalsh(symmetric_block)
        ...
        add_report(
            rows,
            f"{component}_kernel_spectrum",
            "PASS",
            kernel_path,
            f"sampled_n={len(sym_idx)}; positive_eigenvalues={len(positive)}; effective_rank={effective_rank:.6g}; positive_condition={condition:.6g}",
            required,
        )
...
def validate_compact_orders(
    ...
    if kg and kg_rbf and kg["shape"] == kg_rbf["shape"]:
        K1 = np.load(args.compact_g_kernel, mmap_mode="r")
        K2 = np.load(args.compact_g_rbf_kernel, mmap_mode="r")
        idx = sampled_indices(K1.shape[0], min(sample_n, 512))
        tri = np.triu_indices(len(idx), k=1)
        a = np.asarray(K1[np.ix_(idx, idx)], dtype=np.float64)[tri]
        b = np.asarray(K2[np.ix_(idx, idx)], dtype=np.float64)[tri]
        corr = float(np.corrcoef(a, b)[0, 1]) if len(a) > 1 and np.std(a) > 0 and np.std(b) > 0 else float("nan")
        status = "WARN" if math.isfinite(corr) and abs(corr) >= 0.98 else "PASS"
        add_report(rows, "compact_genomic_kernel_redundancy", status, args.compact_g_rbf_kernel, f"sampled_upper_triangle_correlation={corr:.6g}; warn_at_abs>=0.98", False)
    return manifest
...
def main() -> None:
    ...
    full_g = validate_kernel(args.geno_kernel, args.geno_order, "sample_id", "full_K_G", rows, True, args.sample_n, expect_mean_diag_one=True)
    full_g_rbf = validate_kernel(args.geno_rbf_kernel, args.geno_order, "sample_id", "full_K_G_RBF", rows, True, args.sample_n, expect_mean_diag_one=True)
    full_e = validate_kernel(args.env_kernel, args.env_order, "env_id", "full_K_E", rows, True, args.sample_n, expect_mean_diag_one=True)
    if full_g:
