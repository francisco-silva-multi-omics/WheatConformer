...
def copy_or_skip(src: Path, dst: Path, dry_run: bool = False, overwrite: bool = False) -> str:
    if dst.exists():
        if not overwrite:
...
def main() -> None:
    ...
    for src in sorted(args.raw_dir.iterdir(), key=lambda p: p.name.lower()):
        ...
        status = copy_or_skip(src, dst, dry_run=args.dry_run, overwrite=args.overwrite)
